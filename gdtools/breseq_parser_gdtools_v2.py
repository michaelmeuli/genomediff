...
# from: https://github.com/NanamiKubota/NanamiKubota.github.io/blob/main/scripts/breseq_parser_gdtools.py
...
def GD2CSV(directory, output, filename, ancestor):
    for gdout in glob.glob(directory + '/**/data/output.gd', recursive=True):
        ...
        fasta1 = '/home/michael/tmp/gdtools/ref/CU458745.fasta'
        fasta2 = '/home/michael/tmp/gdtools/ref/CU458896.fasta'
        if ancestor is None:
            out = os.path.join(p.parent, "annotated.tsv")
            subprocess.run(['gdtools', 'ANNOTATE', '-e', '-o', out, '-f', 'TSV', '-r', ref, '-r', fasta1, '-r', fasta2, p], stdout=subprocess.DEVNULL)
            li = []
